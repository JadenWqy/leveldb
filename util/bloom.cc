 ...
 public:
  ...
  void CreateFilter(const Slice* keys, int n, std::string* dst) const override {
    ...
    for (int i = 0; i < n; i++) { // 1个key做1个hash值，然后循环k_次放到多个位置
      ...
      // Double hashing: h(key,i)=h1(key)+i*h2(key) mod 2^32
      // h1 is BloomHash, h2 is delta (right rotate 17 bits)
  ...
  bool KeyMayMatch(const Slice& key, const Slice& bloom_filter) const override {
    ...
    const size_t k = array[len - 1];  // 取出k值, incase k is not same as the k_
    if (k > 30) {
